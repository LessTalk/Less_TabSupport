...
public class TabLayout extends HorizontalScrollView {
    ...
    public static final int TYPE_SYSTEM = 1 << 1;
    public static final int TYPE_SYSTEM_AUTO_TEXT_WIDTH = 1 << 2;
    public static final int TYPE_NO_LINE = 1 << 3;
    public static final int TYPE_SYSTEM_AUTO_ANIMATION_TEXT_WIDTH = 1 << 4;
    public static final int TYPE_BEZIER_CIRCLE = 1 << 5;
    ...
    private ArrayList<Integer> mTextWidthList = new ArrayList<>();
    ...
    private void setScrollPosition(int position, float positionOffset, boolean updateSelectedText,
        ...
        if (roundedPosition < 0 || roundedPosition >= mTabStrip.mLinearLayout.getChildCount()) {
            return;
        }
    ...
    public void setView(CommonTabView commonTabView) {
        mTabStrip.addView(commonTabView);
    }
    ...
    public void removeAllTabs() {
        ...
        for (int i = mTabStrip.mLinearLayout.getChildCount() - 1; i >= 0; i--) {
            removeTabViewAt(i);
        }
    ...
    final ViewPager viewPager, boolean autoRefresh) {
        setupWithViewPager(viewPager, autoRefresh, false);
    }

    private void setupWithViewPager(@Nullable
    final ViewPager viewPager, boolean autoRefresh,
    ...
    private void setPagerAdapter(@Nullable
    final TabPagerAdapter adapter, final boolean addObserver) {
    ...
    private void populateFromPagerAdapter() {
        ...
        if (mPagerAdapter != null) {
            ...
            for (int i = 0; i < adapterCount; i++) {
                addTab(newTab().setText(mPagerAdapter.
                        getPageTitle(i)).setTextColor(mPagerAdapter.getPageColor(i)), false);
    ...
    private TabView createTabView(@NonNull
    final Tab tab) {
    ...
    private void addTabView(Tab tab, boolean setSelected) {
        ...
        if (mType == TYPE_SYSTEM_AUTO_TEXT_WIDTH || mType == TYPE_SYSTEM_AUTO_ANIMATION_TEXT_WIDTH || mType == TYPE_BEZIER_CIRCLE) {
            mTextWidthList.add(ViewUtils.getWidth(tabView.mTextView));
        }
        mTabStrip.mLinearLayout.addView(tabView, createLayoutParamsForTabs());
        if (setSelected) {
    ...
    private void addTabView(Tab tab, int position, boolean setSelected) {
        ...
        mTabStrip.mLinearLayout.addView(tabView, position, createLayoutParamsForTabs());
        if (setSelected) {
            tabView.setSelected(true);
        }
    ...
    private void removeTabViewAt(int position) {
        final TabView view = (TabView) mTabStrip.mLinearLayout.getChildAt(position);
        mTabStrip.mLinearLayout.removeViewAt(position);
        if (view != null) {
            view.reset();
    ...
    private void setSelectedTabView(int position) {
        final int tabCount = mTabStrip.mLinearLayout.getChildCount();
        if (position < tabCount && !mTabStrip.mLinearLayout.getChildAt(position).isSelected()) {
            for (int i = 0; i < tabCount; i++) {
                final View child = mTabStrip.mLinearLayout.getChildAt(i);
    ...
    private void dispatchTabSelected(@NonNull
    final Tab tab) {
    ...
    private void dispatchTabUnselected(@NonNull
    final Tab tab) {
        for (int i = mSelectedListeners.size() - 1; i >= 0; i--) {
    ...
    private void dispatchTabReselected(@NonNull
    final Tab tab) {
        for (int i = mSelectedListeners.size() - 1; i >= 0; i--) {
    ...
    private int calculateScrollXForTab(int position, float positionOffset) {
        if (mMode == MODE_SCROLLABLE) {
            final View selectedChild = mTabStrip.mLinearLayout.getChildAt(position);
            final View nextChild = position + 1 < mTabStrip.mLinearLayout.getChildCount()
                    ? mTabStrip.mLinearLayout.getChildAt(position + 1)
                    : null;
    ...
    private void applyModeAndGravity() {
        ...
        switch (mMode) {
            case MODE_FIXED:
                mTabStrip.mLinearLayout.setGravity(Gravity.CENTER_HORIZONTAL);
                break;
            case MODE_SCROLLABLE:
                mTabStrip.mLinearLayout.setGravity(GravityCompat.START);
                break;
        }
    ...
    private void updateTabViews(final boolean requestLayout) {
        for (int i = 0; i < mTabStrip.mLinearLayout.getChildCount(); i++) {
            View child = mTabStrip.mLinearLayout.getChildAt(i);
    ...
    class TabView extends RelativeLayout implements OnLongClickListener {
        private Tab mTab;
        ...
        final void update() {
            ...
            if (mCustomView == null) {
                ...
                if (mTextView == null) {
                    ...
                    layoutParams.addRule(RelativeLayout.CENTER_HORIZONTAL);
                    layoutParams.addRule(RelativeLayout.BELOW,R.id.tab_icon);
                    addView(textView);
                    mTextView = textView;
        ...
        final TextView textView,
            ...
            if (textView != null) {
                if (hasText) {
                    ...
                    if(mTab != null){
                        textView.setTextColor(mTab.getColor());
                    }
    ...
    private static int blendColors(int color1, int color2, float ratio) {
        ...
        return Color.argb(178, (int) r, (int) g, (int) b);
    }

    private class SlidingTabStrip extends FrameLayout {
        ...
        public void addView(CommonTabView commonTabView) {
            View view = getChildAt(0);
            removeView(view);
            LayoutParams layoutParams = new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
            addView(commonTabView, layoutParams);
            mCommonTabView = commonTabView;
        ...
        boolean childrenNeedLayout() {
            for (int i = 0, z = mLinearLayout.getChildCount(); i < z; i++) {
            ...
            if (mMode == MODE_FIXED && mTabGravity == GRAVITY_CENTER) {
                ...
                if (largestTabWidth * count <= getMeasuredWidth() - gutter * 2) {
                    ...
                    for (int i = 0; i < count; i++) {
                        final LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) (mLinearLayout.getChildAt(i).getLayoutParams());
        ...
        protected void onLayout(boolean changed, int l, int t, int r, int b) {
            ...
            if (mCommonTabView != null) {
                mCommonTabView.init((Math.min(getMeasuredWidth(), getMeasuredHeight())) / 2,getMeasuredWidth()/getTabCount(),getMeasuredHeight());
            }
        }

        private void updateIndicatorPosition() {
            final View selectedTitle = mLinearLayout.getChildAt(mSelectedPosition);
            int left, right;
        ...
        private void setIndicatorPosition(int left, int right) {
            if (left != mIndicatorLeft || right != mIndicatorRight) {
                ...
                ViewCompat.postInvalidateOnAnimation(this);
                if (mType == TYPE_BEZIER_CIRCLE) {
                    float tabWidth = getMeasuredWidth() / getTabCount();
                    float interpolatedTime = left / tabWidth;
                    mCommonTabView.setCurrentColor(blendColors(mPagerAdapter.getPageColor(mSelectedPosition),mPagerAdapter.getPageColor(mSelectedPosition+1),interpolatedTime));
                    mCommonTabView.draw(interpolatedTime);
                }
            }
        }
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            final View targetView = mLinearLayout.getChildAt(position);
            if (targetView == null) {
        ...
        private void drawLine(Canvas canvas, float tabWidth) {
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                int value = ((int) tabWidth - mTextWidthList.get(mSelectedPosition)) / 2;
                float temp = (diff * (mIndicatorLeft - tabWidth * mSelectedPosition)) / 2f;
                canvas.drawRect(mIndicatorLeft + temp + value, getHeight() - mSelectedIndicatorHeight,
                        mIndicatorRight - temp - value, getHeight(), mSelectedIndicatorPaint);
            }
        }
        ...
        public void draw(Canvas canvas) {
            super.draw(canvas);
            if (mType == TYPE_NO_LINE) {
                return;
            }
            if (mType == TYPE_SYSTEM_AUTO_ANIMATION_TEXT_WIDTH) {
                float tabWidth = getMeasuredWidth() / mTextWidthList.size();
                if (mIndicatorLeft % tabWidth == 0) {
                    diff = 0;
                    if (mTextWidthList.size() > mSelectedPosition + 1) {

                        int diffValue = mTextWidthList.get(mSelectedPosition) - mTextWidthList.get(mSelectedPosition + 1);
                        if (diffValue != 0) {
                            diff = diffValue / tabWidth;
                            drawLine(canvas, tabWidth);
                        } else {
                            drawLine(canvas, tabWidth);
                        }
                    } else {
            ...
            } else if (mType == TYPE_SYSTEM_AUTO_TEXT_WIDTH) {
                int tabWidth = getMeasuredWidth() / mTextWidthList.size();
                int diff = (tabWidth - mTextWidthList.get(mSelectedPosition)) / 2;
                if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                    canvas.drawRect(mIndicatorLeft + diff, getHeight() - mSelectedIndicatorHeight,
                            mIndicatorRight - diff, getHeight(), mSelectedIndicatorPaint);
                }
            } else if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
    ...
    private class AdapterChangeListener implements ViewPager.OnAdapterChangeListener {
        ...
        public void onAdapterChanged(@NonNull ViewPager viewPager,
            ...
            if (mViewPager == viewPager) {
                setPagerAdapter((TabPagerAdapter)newAdapter, mAutoRefresh);
            }
        }
